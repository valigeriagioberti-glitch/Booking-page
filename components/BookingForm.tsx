...
import { Calendar, Plus, Minus, CreditCard, User, Mail, Phone, Clock, Info, ChevronRight } from 'lucide-react';
...
export const BookingForm: React.FC<BookingFormProps> = ({ onComplete, language }) => {
  ...
  return (
    <div className="relative pb-24 md:pb-0">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-4 z-40 flex items-center justify-between safe-area-inset-bottom shadow-[0_-1px_10px_rgba(0,0,0,0.02)]">
        <div className="flex flex-col">
          <div className="text-lg font-black text-gray-900">€{totalPrice.toFixed(2)}</div>
          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mt-0.5">
            {billableDays} {billableDays === 1 ? t.booking.day : t.booking.days}
          </div>
        </div>
        <button
          onClick={handlePayment}
          disabled={isLoading || !isFormValid}
          className={`px-8 py-3.5 rounded-xl font-bold text-sm uppercase tracking-widest flex items-center space-x-2 transition-all active:scale-95 ${
            isLoading || !isFormValid
            ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
        ...
        >
          {isLoading ? (
            ...
            <>
              <span>{t.booking.payButton.split(' ')[0]}</span>
